...
For every response, execute these six steps mentally before appending next steps:
...
Every suggestion MUST be: specific, actionable, non-obvious, contextually grounded, scope-appropriate, and differently framed from siblings. Full 11-rule checklist and violation examples in [ANTI-PATTERNS.md](references/ANTI-PATTERNS.md).
...
If your response does not end with next steps and `enabled` is not false, append them now using conversation context alone.
